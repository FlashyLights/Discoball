...
class moduleManager {
	constructor(bot) {
		var self = this;
		self.bot = bot;
		self.logging = self.bot.discoball.logger("discoball::moduleManager");
		self.loadedModules = {};
	}
	...
	locateModules() {
		...
		var manifests = new DirLoad(self.bot.config.get('discoball.baseDir') + "/modules", {
		...
		loadedManifests = _.filter(loadedManifests, function(o) {
			...
			return !_.includes(self.bot.config.get('discoball.ignoreModules'), o.module.base)
		});
	...
	initModules() {
		...
		_.each(appModules, function(app) {
			...
			self.logging.info("Loading " + module.name + " by " + module.author.name);
			
			var loadedEntryPoints = {};
			_.each(module.entryPoints, function(entryPoint) {
				var friendly = _.camelCase(entryPoint.replace(/\.(.*?)$/, "").replace(/\//g, " "));

				if (_.includes(self.bot.config.get('discoball.ignoreModules'), path.join(module.base, entryPoint))) {
					return;
				}
				...
				try {
					...
					self.logging.error("Entrypoint " + entryPoint + " was not found. This is likely due to a malformed module.json");
				}
				...
				if (_.isFunction(loadedEntryPoints[friendly].preInit)) {
					...
					self.logging.silly("Called pre-init on " + friendly);
				}
			});
		...
		_.each(self.loadedModules, function(module) { 
			_.each(module.classes, function(loadedClass, friendly) {
				if (_.isFunction(loadedClass.init)) {
					loadedClass.init();
					self.logging.info("Called init on " + friendly);
				}
			});
		});
		...
		_.each(self.loadedModules, function(module) {
			_.each(module.classes, function(loadedClass, friendly) {
				if (_.isFunction(loadedClass.postInit)) {
					...
					self.logging.info("Called post-init on " + friendly);
				}
			});
		});
